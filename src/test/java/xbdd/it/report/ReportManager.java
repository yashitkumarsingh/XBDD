...
import xbdd.it.XbddDriver;
import xbdd.it.utils.JerseyClientFactory;
...
public class ReportManager {
	...
	public void loadReport(final String resource, final String rootURL, final ReportContext reportContext) throws IllegalArgumentException,
		...
		try (final InputStream is = this.getClass().getClassLoader().getResourceAsStream(resource)) {
			...
			//System.out.println(IOUtils.toString(this.getClass().getClassLoader().getResourceAsStream(resource), "UTF-8"));
	...
	public String getAttachment() {
		final String src = new WebDriverWait(this.webDriver, 5).until(
				ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".scenario-steps img"))).getAttribute("src");
		final Long status = (Long) ((JavascriptExecutor) this.webDriver)
				.executeScript("var req = new XMLHttpRequest(); req.open('get',arguments[0], false); req.send(null); return req.status",
						src);
		Assert.assertThat("Image not found", status, Matchers.lessThan(300l));
		return src;
	}
